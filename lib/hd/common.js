...
/**
 * Generate stringderivation path from array of indexes
 * @param {Number[]} indexes
 * @returns {String}
 */

common.toPath = function toPath(indexes) {
  ...
  for (const index of indexes) {
    const hardened = index & common.HARDENED;

    if (hardened)
      path += `/${index ^ common.HARDENED}'`;
    else
      path += `/${index}`;
  }
